        ...
        try:
            ...
            for cmd in commands:
                ...
                if action == "delay":
                    ...
                    # Change reasoning effort level mid-session via /effort slash command.
                    ...
                    if new_level in valid_levels:
                        ...
                        if current_level == new_level:
                            ...
                            # Level change — send /effort slash command (no restart)
                            old_level = current_level or "default"
                            print(f"[asdaaas] REASONING EFFORT: {old_level} -> {new_level}, sending /effort...")
                            try:
                                await backend.send_prompt(f"/effort {new_level}")
                                await backend.collect_response(backend._rpc_id, keepalive_timeout=10.0, max_wall_clock=30.0)
            ...
            if deliver_result is not None:
                ...
                if reasoning_effort_turns_remaining is not None:
                    ...
                    if reasoning_effort_turns_remaining <= 0:
                        # Auto-revert to default via /effort slash command
                        ...
                        if current_level != revert_to:
                            ...
                            try:
                                await backend.send_prompt(f"/effort {revert_to}")
                                await backend.collect_response(backend._rpc_id, keepalive_timeout=10.0, max_wall_clock=30.0)
