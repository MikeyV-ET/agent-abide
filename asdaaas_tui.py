...
# =============================================================================
# Color Palette — Gruvbox (Dark + Light)
# =============================================================================
...
_GRUVBOX_DARK = {
    ...
    "BR_BLUE": "#83a598", "BR_PURPLE": "#d3869b", "BR_AQUA": "#8ec07c",
    "BR_ORANGE": "#fe8019",
...
_GRUVBOX_LIGHT = {
    ...
    "BR_BLUE": "#076678", "BR_PURPLE": "#8f3f71", "BR_AQUA": "#427b58",
    "BR_ORANGE": "#af3a03",
...
class Gruvbox:
    """Gruvbox color palette. Defaults to dark; call set_light() before app starts."""
    BG = _GRUVBOX_DARK["BG"]
    FG = _GRUVBOX_DARK["FG"]
    GRAY = _GRUVBOX_DARK["GRAY"]
    RED = _GRUVBOX_DARK["RED"]
    GREEN = _GRUVBOX_DARK["GREEN"]
    YELLOW = _GRUVBOX_DARK["YELLOW"]
    BLUE = _GRUVBOX_DARK["BLUE"]
    PURPLE = _GRUVBOX_DARK["PURPLE"]
    AQUA = _GRUVBOX_DARK["AQUA"]
    ORANGE = _GRUVBOX_DARK["ORANGE"]
    BR_RED = _GRUVBOX_DARK["BR_RED"]
    BR_GREEN = _GRUVBOX_DARK["BR_GREEN"]
    BR_YELLOW = _GRUVBOX_DARK["BR_YELLOW"]
    BR_BLUE = _GRUVBOX_DARK["BR_BLUE"]
    BR_PURPLE = _GRUVBOX_DARK["BR_PURPLE"]
    BR_AQUA = _GRUVBOX_DARK["BR_AQUA"]
    BR_ORANGE = _GRUVBOX_DARK["BR_ORANGE"]
    DARK1 = _GRUVBOX_DARK["DARK1"]
    DARK2 = _GRUVBOX_DARK["DARK2"]
    DARK3 = _GRUVBOX_DARK["DARK3"]
    DARK4 = _GRUVBOX_DARK["DARK4"]

    @classmethod
    def set_light(cls):
        """Switch all color attributes to the Gruvbox light palette."""
        for key, val in _GRUVBOX_LIGHT.items():
            setattr(cls, key, val)
...
class GazeSelector(OptionList):
    ...
    def on_option_list_option_selected(self, event: OptionList.OptionSelected) -> None:
        ...
        if room in AsdaaasTUI._DIRECT_ADAPTERS:
            try:
                self.app._ensure_adapter_attached(room)
            except Exception:
                pass
        self.app._set_gaze_to_room(room)
        self.display = False
...
class MessageInput(TextArea):
    ...
    def action_undo(self) -> None:
        """Workaround for Textual bug: _undo_batch calls _refresh_size() before
        restoring cursor positions via edit.after(). When a multiline undo
        shrinks the document, the stale cursor points past the last line and
        location_to_offset raises ValueError. Catch and clamp."""
        try:
            super().action_undo()
        except ValueError:
...
class ToolCallPanel(Static):
    ...
    def render(self):
        ...
        title = f"{kind_icon} {rich_escape(self.tool_title)} {status_icon}"
    ...
    def on_message_input_submitted(self, event: MessageInput.Submitted) -> None:
        ...
        if self._room_active:
            ...
            try:
                ...
                pass
            # Send via IRC socket directly (fast — just a socket write)
            import threading
...
"""
    ...
    # Adapters that are their own gaze target (not routed through IRC)
    _DIRECT_ADAPTERS = {"tui", "arena", "meet"}

    def _set_gaze_to_room(self, room: str) -> None:
        """Set gaze by writing gaze.json directly. Single source of truth for gaze changes."""
        agent_lower = self._active_agent.lower()
        if room in self._DIRECT_ADAPTERS:
            gaze = {
                "speech": {"target": room, "params": {}},
                "thoughts": {"target": "irc", "params": {"room": f"#{agent_lower}-thoughts"}}
            }
            gaze_str = room
        elif room.startswith("pm:") or room.startswith("#"):
            gaze = {
    ...
    def _connect_room_irc(self):
        ...
        # Start keepalive thread to handle PINGs
    ...
    def _tail_room_log(self) -> None:
        ...
        # Parse format: [2026-04-05 06:13:42 UTC] <nick> message
        # Or: [2026-04-05 06:13:42 UTC] * nick action
        # Or: [2026-04-05 06:13:42 UTC] * nick joined/quit/left
        msg_re = re.compile(r"^\[([^\]]+)\] <([^>]+)> (.*)$")
        ...
        with open(log_path, "r") as f:
            lines = f.readlines()
            # Show last 50 lines as scrollback
            ...
            while True:
                ...
                if m:
                    ts, nick, msg = m.groups()
                    # Skip own messages (already echoed locally on send)
    ...
    def _load_older_history(self, agent_name: str = None) -> None:
        ...
        try:
            ...
            self.notify(f"History error: {e}", severity="error", markup=False)
        finally:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Use Gruvbox light color scheme (default: dark)"
    )
